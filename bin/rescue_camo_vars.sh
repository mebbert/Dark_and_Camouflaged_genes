...
do
	...
	# Create a .bed file for each ploidy (n repeated regions * 2)
	for repeat_num in $(seq 2 1 $max_repeat)
	do
		ploidy=$(( 2 * $repeat_num ))
		...
		# samtools on the command line. For some reason, samtools does not use the .bam index
