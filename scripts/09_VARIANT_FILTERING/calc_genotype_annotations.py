...
def calcGenotypeVals(genotypes):
	...
	m = np.array([ogeno, egeno])
	inbreedingCoeff = round(1 - (sum(ogeno[1:ploidy]) + 1) / (sum(egeno[1:ploidy])+ 1), 5)
...
def updateVcf(vcf):
	...
	for line in vcf:
		...
		inbreeding = calcGenotypeVals(genos)
