...
def extractVariantMetrics(vcf):
	...
	for line in vcf:
		...
		print('\t'.join([chrom, str(pos - 1), str(pos), variant_type, subtype, PASS, info['set'],
			info['AF'], info['QD'], info['FS'], info['SOR'], info['MQ'], info['MQRankSum'],
			info['ReadPosRankSum'], info['BaseQRankSum'], info['ClippingRankSum'],
