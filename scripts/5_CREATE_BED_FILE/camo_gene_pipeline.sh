...
#Merge coordinates for mapq and depth beds
...
bedtools intersect \
	...
	python annotate_regions.py \
		$percent_dark $biotype_dark $coding_dark | \
		bedtools sort -i - -g $faidx \
		> $dark_annotations
...
bedtools intersect \
	...
	-loj | \
	python annotate_regions.py \
		$percent_depth $biotype_depth $coding_depth | \
		bedtools sort -i - -g $faidx \
		> $depth_annotations
...
bedtools intersect \
	...
	-loj | \
	python annotate_regions.py \
		$percent_mapq $biotype_mapq $coding_mapq| \
		bedtools sort -i - -g $faidx \
		> $mapq_annotations
...
bedtools intersect \
	...
	-loj | \
	python annotate_regions.py \
		$percent_camo $biotype_camo $coding_camo | \
		bedtools sort -i - -g $faidx \
		> $camo_annotations
...
mkdir -p $RESULT_DIR
mv $mapq_merged $depth_merged $dark_merged \
